...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    Walk each breaker's transition_log:
    - Find "open" → "closed" pairs
    - Compute delta in ms for each pair
    - Return average, or None if no recovery occurred.
    """
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            # Pass if fallback_success_rate is high (primary is down, fallback should handle)
            passed = result.fallback_successes > 0 or result.cache_hits > 0
        elif scenario.name == "all_healthy":
            # Pass if most requests go through primary (low error rate)
